...
MainWindow::MainWindow(QWidget *parent) :
    ...
    connected(false), plotting(false), autoScale(true), dataPointNumber(0), gCounter(0), dutyCycle(100), periodS(1), customCmd(255), STATE(WAIT_START), NUMBER_OF_POINTS(500), currentNode(0), prevTime(QDateTime::currentMSecsSinceEpoch())
{
    ui->setupUi(this);
    ...
    //avgVecTmp = new QVector<int>(500);
    //avgVecDie = new QVector<int>(500);
    avgCounter = 0;
}
/******************************************************************************************************************/
    ...
    if(serialPort->bytesAvailable()) {                                                    // If any bytes are available
        ...
        if((dsize % 24) == 0) {                                                                 // If the byte array has a whole packet
            ...
            for(i=0; i<msgClusteredPkts; i++){
                ...
                QString packet;
                packet.sprintf("%d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d; %d", upkPld.sinkId, upkPld.packetType, upkPld.packetLength,
...
void MainWindow::writeCmd2Device(int code){
    ...
    serialPort->waitForBytesWritten(000);
...
void Payload::unpack(QByteArray data){
    ...
    dielectric = this->fetch(&temp[8], BYTE2X);
    temperature = this->fetch(&temp[10], BYTE2X);
    packetsSent = this->fetch(&temp[12], BYTE2X);
    packetsLost = this->fetch(&temp[15], BYTE2X);
    rssi = this->fetch(&temp[19], BYTE2X);
    rtt = this->fetch(&temp[17], BYTE2X);
    memoryPage = this->fetch(&temp[21], BYTE3X);
}

int Payload::fetch(char *data, int size){
    int i;
    int auxInt = 0;
    for(i=0; i<size; i++){
        auxInt += data[i];
